...
int convert2_profile(audio_devices_t device, int vcm_param)
{
    switch( device )
    {
        ...
        case AUDIO_DEVICE_IN_BLUETOOTH_SCO_HEADSET:
            ...
            {
                if ( vcm_param & VCM_BT_NREC_OFF )
                    return 10;//PROFILE_EXTRA_VOL|PROFILE_BT_NREC_OFF;
                return 9;//PROFILE_EXTRA_VOL|PROFILE_BT_WB;
            }
            if ( vcm_param & VCM_BT_NREC_OFF )
                return 8;//PROFILE_EXTRA_VOL;
            return 4;//PROFILE_BT_STEREO;

        case AUDIO_DEVICE_OUT_EARPIECE:
        case AUDIO_DEVICE_IN_BUILTIN_MIC:
        case AUDIO_DEVICE_IN_BACK_MIC:
            if( vcm_param & VCM_DUAL_MIC )
                return 11;//PROFILE_EXTRA_VOL|PROFILE_BT_NREC_OFF|PROFILE_BT_WB;
            if( vcm_param & VCM_EXTRA_VOL )
                return 14;//PROFILE_EXTRA_VOL|PROFILE_BT_STEREO|PROFILE_BT_NREC_OFF;
            return 1;//PROFILE_BT_WB;
            
        case AUDIO_DEVICE_OUT_ANLG_DOCK_HEADSET:
        case AUDIO_DEVICE_OUT_DGTL_DOCK_HEADSET:
            return 2;//PROFILE_BT_NREC_OFF;

        case AUDIO_DEVICE_IN_WIRED_HEADSET:
        case AUDIO_DEVICE_OUT_WIRED_HEADSET:
            if ( vcm_param & VCM_TTY_FULL )
                return 18;//PROFILE_LOOP|PROFILE_BT_NREC_OFF;
            if ( vcm_param & VCM_TTY_HCO )
                return 19;//PROFILE_LOOP|PROFILE_BT_NREC_OFF|PROFILE_BT_WB;
            if ( vcm_param & VCM_TTY_VCO )
                return 20;//PROFILE_LOOP|PROFILE_BT_STEREO;
            if ( vcm_param & VCM_TTY_VCO_DUALMIC )
                return 21;//PROFILE_LOOP|PROFILE_BT_STEREO|PROFILE_BT_WB;
            return 2;//PROFILE_BT_NREC_OFF;
            
        case AUDIO_DEVICE_OUT_WIRED_HEADPHONE:
            return 7;//PROFILE_BT_STEREO|PROFILE_BT_NREC_OFF|PROFILE_BT_WB
